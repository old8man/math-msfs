...
\paragraph{Machine verification.}  Every theorem, lemma, and corollary in this paper has a companion machine-checked formal proof in the \textbf{Verum MSFS Corpus} (\url{https://github.com/gst-st/msfs}), self-contained modulo ZFC + 2 strongly inaccessible cardinals.  See Appendix~\ref{app:verification-ledger} for a one-paragraph entry-point and follow the corpus repository for the live audit surface, paper-to-corpus map, and reproducibility instructions.
...
\begin{proof}
$\phi_X$ presents $X \in \Ob(\Syn(F))$ for some sub-theory $\iota_F : F \hookrightarrow S$ with $F \in \cF$ (using the Definition~\ref{def:F} sub-theory inclusion notation rather than a set-theoretic ``$F \subseteq S$'').  By (R2), (R5a), $\Syn(F)$ is the Grothendieck construction over $\Mod(F)$ (Lurie~\cite{LurieHTT} \S 5.1), a derived functor from $\cS_S^{\mathrm{local}}$-data, so $\Syn(F) \in \cS_S^{\mathrm{global}}$; hence $X \in \cS_S^{\mathrm{global}} \subseteq \cS_S$.
\end{proof}
...
\paragraph{Topos tower.} $A_0 = $ a $1$-topos; $A_1 = 2$-topos (Shulman~\cite{Shulman2008}); $A_n = (\infty, n)$-topos. The limit $A_\infty$ — the stabilized $(\infty, \infty)$-topos — is defined here in the Barwick--Schommer-Pries~\cite{BarwickSchommerPries2021} unicity sense applied to the $A_n$ tower; the $(\infty, 1)$-stratum follows Lurie~\cite{LurieHTT}, the $(\infty, 2)$-stratum Shulman~\cite{Shulman2008}, and the higher strata are an ongoing program.
...
\emph{Morita equivalence}: $F_1 \sim_M F_2$ at the level of $(1, 1)$-categorical models follows from Hofmann's conservativity theorem (Hofmann~\cite{Hofmann1995}, Chapter~3): the syntactic translation between $\MLTT + \mathsf{UIP}$ and $\ETT$ is provably conservative, and lifts to a Morita-equivalence of their $(1, 1)$-categorical model categories on the class of locally cartesian closed categories where the relevant identity-type semantics is well-defined.
...
are equivalences of $(\infty, \infty)$-categories: since accessible-functor categories preserve equivalences in the target $(\infty, \infty)$-cat (and the accessibility constraint is invariant under the $(\infty, n) \hookrightarrow (\infty, n+1)$ truncation jumps), Theorem~\ref{thm:bergner-lurie-stab}'s equivalence on $(\infty, \infty)$-$\Cat$ itself transfers to the accessible-presheaf category $\Pi_{(\infty, \infty)} = \Fun^{\mathrm{acc}}(\fM, (\infty, \infty)\text{-}\Cat)$. Therefore the canonical $2$-functors
...
\textbf{(c)}: Suppose, for contradiction, $\fM^{(\mathrm{Cls}\cdot k)}$ (for any $k \geq 1$) satisfies the $\LAbs$ conditions $(F_S) \wedge (\Pi_{4, S, n}) \wedge (\Pi_{3\text{-max}, S, n})$ (Definition~\ref{def:level6}). By (b), $\fM^{(\mathrm{Cls}\cdot k)}$ realizes the same $(\infty, \infty)$-theory as $\fM^{(\mathrm{Cls})} \in \Meta_{\mathrm{Cls}}$ (though at a higher Grothendieck-universe level $\kappa_k$). As a $\LCls$ meta-structure, $\fM^{(\mathrm{Cls}\cdot k)}$ is classifying but not generative of Rich-foundations (M4), contradicting $\LAbs$ maximal generativity $(\Pi_{3\text{-max}, S, n})$. The contradiction shows no $\LAbs$ escalation occurs.
\end{proof}
...
\begin{proof}
By Theorem~\ref{thm:ac-oc-duality}, every statement about $\fM_\cE$ translates to an equivalent statement about $\fM$ via the equivalence. No new axioms are introduced; by construction, $\cE$ is $\mathbf{U}_2$-small and is constructed inside $\ZFC + 2\text{-inacc}$ from $\cF$ and $\mathbf{StrCat}_{S, n}$ (Definition~\ref{def:strcat}) using only Lambek--Scott adjunctions guaranteed by (R5b) and reflectors given as object data (Definition~\ref{def:enactments}).
\end{proof}
...
Theorems~\ref{thm:horizontal}--\ref{thm:completeness} transfer to $\LAbs^\cE$: the emptiness of $\LAbs^\cE$ is absolute along the five structural axes
\begin{center}
...
\item \emph{Higher topos theory (Lurie)}: passes $(F_S)$, fails $(\Pi_{3\text{-max}, S, n})$: classifies categorical structures of HoTT-compatible shape but does not admit a classification functor onto Rich-foundations outside that class (e.g.\ first-order set theories without a HoTT-style internal language).
...
Every theorem, lemma, definition, and corollary in this paper has a
companion machine-checked formal proof in the \textbf{Verum MSFS
Corpus}, available at \url{https://github.com/gst-st/msfs}.  The
companion artefact ships its own README with the full paper-to-corpus
map, the trusted-boundary report, the live audit surface
(\texttt{verum audit -\!-bundle} produces a one-command verdict),
foreign-format exports (Coq, Lean~4, Agda, Dedukti, Metamath), and
reproducibility instructions.

\medskip

\noindent\textbf{Trusted boundary.}  The MSFS verification is
self-contained modulo:
\begin{itemize}
\item ZFC,
\item two strongly inaccessible cardinals $\kappa_1 < \kappa_2$,
\item the Verum trusted kernel (\texttt{crates/verum\_kernel/} of the
      Verum source tree).
\end{itemize}
Nothing else is admitted.  The boundary is enforced by the
kernel-side invariant
\texttt{verum\_kernel::mechanisation\_roadmap::msfs\_self\_contained()}
and by the corpus-side Verum-language theorem
\texttt{msfs\_self\_containment\_theorem}; both are re-checked on every
build and refuse any commit that introduces an external dependency
within MSFS scope.

\medskip

\noindent\textbf{Citations to external authorities} (Lurie HTT 2009,
Ad\'amek--Rosick\'y 1994, Bergner--Lurie 2021, Riehl--Verity 2022,
Whitehead, Pronk's bicategory-of-fractions, Lawvere's fixed-point
theorem) appear in the corpus as
\texttt{@framework(name, "citation")} markers and are enumerated by
\texttt{verum audit -\!-framework-axioms}.
