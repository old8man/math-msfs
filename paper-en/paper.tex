...
\item Derived functors (in the Quillen sense) between $\cS_S^{(k)}$-objects presented as model categories;
...
\end{tabular}
\end{center}
In each row, AFN-T at $(S_T, n_T)$ yields $\LAbs = \emptyset$ restricted to the row's maximality aspect; the row's classical diagonal is a specific witness refinement of $\id_{X_T}$.
\end{theorem}
