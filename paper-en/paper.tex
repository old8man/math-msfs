...
\end{itemize}
These are not linearly ordered: definitional equivalence is strict (requires a common sub-theory), Leibniz equivalence requires formula-bijection, while Morita and bi-interpretability identify theories by model-categorical or interpretation-categorical equivalence. In standard examples, Morita equivalence is strictly \emph{coarser} than definitional equivalence: $\ZFC$ and $\mathrm{ETCS}$ are Morita-equivalent (their categories of sets are equivalent) but not definitionally equivalent (they do not share a common sub-theory with matching definable classes). The paper adopts Morita-reducibility at the $(\infty, n)$-level for two structural reasons. First, Morita equivalence is the \emph{natural equivalence} on categories of models, corresponding precisely to the syntactic Lambek--Scott adjunction of (R5b); the syntax--semantics bridge of Lemma~\ref{lem:SS-membership} is definable in exactly this framework. Second, AFN-T's structural content --- the syntax--semantics closure via the identity morphism $\id_X$ --- is stable under the choice of equivalence: any reflexive equivalence relation respecting composition (in particular all four listed above: Morita, bi-interpretability, definitional, Leibniz) admits the identity $\id_X : X \to X$ as a self-reducing morphism. Replacing Morita with a strictly finer equivalence (e.g., definitional) yields a strictly finer moduli space with strictly more points --- Theorem~\ref{thm:afnt-alpha} applied with this finer relation still vacates $\LAbs$ because $\id_X$ remains a reducing morphism regardless of the specific equivalence notion; the contradiction at $(F_S) \wedge (\Pi_4)$ is independent of whether $\id_X$ witnesses Morita-reduction, bi-interpretable-reduction, or definitional-reduction. Conversely, replacing Morita with a strictly coarser equivalence yields a coarser moduli with fewer points, and the same argument applies. Incompatible equivalence choices --- those failing to respect the syntax--semantics adjunction of (R5b) (e.g., equivalences peculiar to paraconsistent logics that allow true contradictions) --- are not covered by Theorem~\ref{thm:afnt-alpha} and are addressed in the paraconsistent extension (Appendix~\ref{app:paraconsistent}). In summary: the choice of Morita is not \emph{free}; it is dictated by (R5b) as the natural categorical equivalence compatible with Lambek--Scott semantics, and AFN-T's conclusion is stable across all reflexive composition-respecting equivalences relating consistent R-S theories.
\end{remark}
...
\item[\textbf{(R5b)}] the syntax-to-semantics evaluation $\ev : \Syn(S) \to \cM$ for every model $\cM \models S$ is an $S$-preserving $(\infty, n_S)$-functor, and the pair of functors $(\Syn, \Mod)$ constitutes a Lambek--Scott-type adjunction whose unit at $\Syn(S)$ is an $(\infty, n_S)$-equivalence onto the \emph{initial $S$-structured $(\infty, n_S)$-category}, i.e., the initial object in the $(\infty, n_S + 1)$-category of $S$-structured $(\infty, n_S)$-categories and $S$-preserving $(\infty, n_S)$-functors between them (equivalently, the $(\infty, n_S)$-category obtained from the freely-generated $S$-structured category by Morita completion). The initial object exists uniquely up to canonical $(\infty, n_S)$-equivalence by the standard construction (Lurie HTT~\cite{LurieHTT} \S 5.5.1 for the $(\infty, 1)$-case; the $(\infty, n)$-version follows by BSP unicity).
\end{itemize}
...
Instances verifying (R5a)--(R5b): first-order $S$ at $n_S = 1$ (Lambek--Scott~\cite{LambekScott1986}); Martin-L\"of type theory with $n_S = \infty$ in the iterative sense above (Awodey--Warren~\cite{AwodeyWarren2009}); HoTT with $n_S = \infty$ likewise (Kapulkin--Lumsdaine~\cite{KapulkinLumsdaine2021}); dependent type theories (Jacobs~\cite{Jacobs1999}); the internal language of $(\infty, 1)$-topoi admits a partial correspondence (Lurie HTT~\cite{LurieHTT} \S 5--\S 6 for the model-categorical presentation; Shulman~\cite{Shulman2019} for univalent universes in every Grothendieck $\infty$-topos, which supplies enough of the internal-language fragment for Lambek--Scott adjunction at the level required by this paper). The full internal-language correspondence for non-truncated $(\infty, 1)$-topoi is an active research area; the argumentation in this paper relies only on the partial correspondence sufficient for (R5b). The condition (R5) does not fix $n_S$: different R-S may have different intrinsic depths, and AFN-T is absolute over this choice (Theorem~\ref{thm:vertical}).
\end{itemize}
...
\begin{remark}[Scope of non-triviality: $\Sint^{\mathrm{eff}}$ vs.\ full $\Sint$]\label{rem:intensional-eff-transfer}
The non-triviality of $\mathrm{Int}([F])$ in Theorem~\ref{thm:slice-locality}(b) is established by exhibiting $\II(\MLTT)$ and $\II(\ETT)$ as non-equivalent in the $\mathrm{Eff}$-restricted sub-$2$-category $\Sint^{\mathrm{eff}}$ (Theorem~\ref{thm:I-existence}, Step 7). Whether these two display classes remain non-equivalent in the \emph{full} $\Sint$ (permitting non-computable $2$-equivalences) is a separate question. Three cases are logically exhaustive:
\begin{enumerate}[nosep,label=(\roman*)]
\item \emph{$\II(\MLTT) \not\simeq \II(\ETT)$ in full $\Sint$}: slice-locality conclusion (a)--(c) holds as stated, with the intensional fibre $\mathrm{Int}([\MLTT])$ containing at least two non-equivalent classes.
\item \emph{$\II(\MLTT) \simeq \II(\ETT)$ in full $\Sint$ via a non-computable $2$-equivalence, but not in $\Sint^{\mathrm{eff}}$}: the intensional-refinement bypass is tracked in $\fM$ at the coarser $\Sint^{\mathrm{eff}}$-level; slice-locality still holds (the fibre structure is well-defined), but the fibre non-triviality weakens to ``non-trivial in $\Sint^{\mathrm{eff}}$, potentially trivial in $\Sint$''. AFN-T-preservation under intensional refinement is unaffected because $\fM$-base remains untouched either way.
\item \emph{$\II(\MLTT) \simeq \II(\ETT)$ in $\Sint^{\mathrm{eff}}$}: excluded by Step 7 via computability argument.
\end{enumerate}
The paper's \emph{structural} conclusion --- that intensional refinement fibers over $\fM$ without introducing new base points --- is model-independent: it follows from (I-4) Morita-as-$2$-localization, which holds in full $\Sint$ regardless of computability restriction. The \emph{non-triviality} of fibres is specific to $\Sint^{\mathrm{eff}}$; the transfer of slice-locality from $\Sint^{\mathrm{eff}}$ to $\Sint$ is structural (preserved by inclusion $\Sint^{\mathrm{eff}} \hookrightarrow \Sint$), while the non-triviality witness is not automatically transferred. The AFN-T conclusion (intensional refinement does not escape) holds in either reading.
\end{remark}
...
\emph{(Max-3) and impredicative R-S.} (Max-3) depth-stratification is a constraint on the \emph{internal} structure of $\mathbf{F}$ as a classifier, \emph{not} on what $\mathbf{F}$ classifies. The classification functor $\Cl_{\mathbf{F}} : \mathbf{F} \to \fM$ maps internal $\mathbf{F}$-objects to gauge-classes of Rich-foundations in $\fM$; gauge-classes of impredicative R-S points (such as $\CIC$ with impredicative universe $\mathsf{Prop}$, or $\mathsf{MK}$ with unrestricted class comprehension) are perfectly valid elements of $\fM$ and are selected by $\Cl_{\mathbf{F}}(A)$ for appropriate $A \in \mathbf{F}$ without requiring $A$ itself to be impredicative. The gauge-quotient structure absorbs the impredicativity of the classified theories while keeping $\mathbf{F}$'s internal machinery depth-stratified. Consequently, the existence of impredicative R-S points in $\fM$ is compatible with (Max-1) full classification and (Max-3) depth stratification acting on $\mathbf{F}$. The potential tension between (Max-1) and (Max-3) suggested by the coexistence of impredicative $\fM$-points and predicative $\mathbf{F}$-structure is dissolved by this scope distinction; the openness of Question~1 (\S\ref{sec:consequences}) concerning the non-emptiness of $\LClsMax$ is a separate existential question not reducible to a (Max-1)--(Max-3) incompatibility.
\end{remark}
...
\emph{Remark on the straightening in the $(\infty, 2)$-setting.} Lurie HTT~\cite{LurieHTT} \S 3.2 establishes the straightening--unstraightening equivalence in the $(\infty, 1)$-categorical setting, where Cartesian fibrations over a fixed base correspond to functors into the $(\infty, 1)$-category of $(\infty, 1)$-categories. The $(\infty, 2)$-version required here --- straightening of Cartesian fibrations between $(\infty, 2)$-categories into $2$-functors into $(\infty, 2)\text{-}\Cat$ --- is a direct extension obtained by iterated application of the $(\infty, 1)$-straightening to the Hom-spaces of $(\infty, 2)$-categories (each Hom-space is itself an $(\infty, 1)$-category, and Cartesian fibrations of $(\infty, 2)$-categories are equivalently Cartesian fibrations pointwise on Hom-spaces). Alternatively, the Barwick--Schommer-Pries~\cite{BarwickSchommerPries2021} unicity theorem for $(\infty, n)$-categories, combined with the Bergner--Rezk~\cite{BergnerRezk2013} model comparison between complete Segal spaces and Joyal's quasi-category model, transfers the $(\infty, 1)$-straightening results uniformly to the $(\infty, 2)$-setting. The universal property invoked in Step 2 is accordingly model-independent by BSP unicity, and the construction of $\mathbf{F}_S^*$ below proceeds uniformly in whichever model is convenient. By construction, $\mathbf{F}_S^*$ is a locally small $2$-category, its classification functor $\Cl_{\mathbf{F}_S^*}$ is essentially surjective onto $\fM$ (so (Max-1) is satisfied), and its $2$-automorphism group $\mathrm{Aut}_2(\mathbf{F}_S^*)$ is the canonical gauge action of $\fM$ on itself (so (Max-2) is satisfied).
...
\begin{remark}[Scope of the non-equivalence claim in Step 2]\label{rem:meta-mult-scope}
The pairwise non-equivalence argument in Step 2 concerns \emph{structure-preserving} $2$-equivalence within $\Meta_{\mathrm{Cls}}$: a $2$-equivalence $\Phi : \mathbf{F}_i \xrightarrow{\simeq_2} \mathbf{F}_j$ in the sense of Definition~\ref{def:meta} is required to preserve the classification functor $\Cl_{\mathbf{F}}$ (up to canonical natural $2$-isomorphism), since $\Cl$ is part of the (M2) structure each $\mathbf{F}$ carries. Under this structure-preservation constraint, $\Phi$ induces a bijection of classification images, and distinct images imply non-equivalence. The argument does \emph{not} rule out bare-$2$-category equivalence between the underlying $2$-categories ignoring classification data; such an equivalence, if it existed, would not be a $\Meta_{\mathrm{Cls}}$-morphism in the sense of Definition~\ref{def:meta} and is therefore not relevant to the claim. The non-equivalence result in Theorem~\ref{thm:meta-mult} is accordingly about non-equivalence within the $\LCls$-stratum with its specified morphism class, not about bare categorical equivalence.
\end{remark}
...
\begin{remark}[Object-centric vs.\ action-centric viewpoints: formal-organizational reconciliation]\label{rem:oc-ac}
Theorem~\ref{thm:ac-oc-duality} formalizes, within the MSFS setting, a \emph{formal-organizational} reconciliation of two classically opposed viewpoints on mathematical foundations:
\begin{itemize}
...
\end{itemize}
The Lambek--Scott adjunction of (R5b), assembled into the pair $(\varepsilon, \alpha)$, exhibits these two viewpoints as $(\infty, \infty)$-equivalent descriptions of a single moduli stack. Neither is primary within the formal setting; the choice between $\cF$ and $\cE$ is a choice of presentation of the same underlying classifying $2$-stack.

\emph{Scope and limits of this reconciliation.} The Morita equivalence $\fM \simeq \fME$ (Theorem~\ref{thm:ac-oc-duality}) is a categorical-structural fact, not a resolution of philosophical disputes about the primacy of action vs.\ object in the philosophy of mathematics. The construction of $\cE$ in Definition~\ref{def:enactments} deliberately restricts enactments to \emph{reflective} quadruples $(F, \cC, \iota, r)$ where the reflector $r : \cC \to \Syn(F)$ is chosen as part of the data; this structural constraint ties $\cE$ tightly to the syntactic core $\Syn(F)$ and is what makes the Morita equivalence formally tractable. Action-centric philosophical positions that reject this syntactic core as primary --- including phenomenological, pragmatist, or enactivist views of mathematics in which \emph{no} syntactic theory is foundationally privileged --- are not captured by $\cE$ and are not addressed by Theorem~\ref{thm:ac-oc-duality}. The pre-formal example in Example~\ref{ex:ac-preformal} illustrates precisely this: such philosophical action-centricity \emph{does not} automatically determine an object of $\cE$ without the discipline of formal theory-reflector-enactment specification. Theorem~\ref{thm:dual-afnt} accordingly expresses the \emph{categorical symmetry} of the no-go conclusion of AFN-T across the $\cF$--$\cE$ duality within the formal setting; it does not claim that the no-go transfers to every conception of ``action-centric foundation'' in the philosophical literature.
\end{remark}
...
AFN-T as stated (Theorem~\ref{thm:afnt}) applies to classical Rich-metatheories. This appendix extends the result to paraconsistent Rich-metatheories \emph{with extractable classical kernel} --- the class of paraconsistent logics in which every theorem admits a classical-inference witness. The extension does \emph{not} cover strongly-dialetheic theories (see Remark~\ref{rem:paraconsistent-scope} below).
...
\begin{remark}[Scope and limitation: strongly-dialetheic theories are outside]\label{rem:paraconsistent-scope}
The extractability condition in Definition~\ref{def:paraconsistent-rs} is \emph{substantive}: it excludes theories whose foundational content \emph{essentially requires} true contradictions (dialetheia) for its formulation. In such strongly-dialetheic theories --- for instance, na\"ive LP without a consistency theorem, or dialetheic set theories where Russell's class is a legitimate object of study \emph{qua} contradiction --- a $\LAbs$ candidate $X$ might be non-Morita-reducible precisely because its non-reducibility witnesses require dialetheia-valid inferences, which the classical projection $X^*$ loses. The extension developed below does not cover such theories; for them, AFN-T's structural content needs to be re-derived in a genuinely dialetheic categorical framework (e.g., using paraconsistent categorical logic in the sense of Estrada-Gonz\'alez and related programmes), which is beyond the present scope. The condition ``extractable classical kernel'' characterizes precisely the paraconsistent theories for which AFN-T's classical-projection argument is available; the complementary class (strongly-dialetheic R-S) is an open problem.
\end{remark}
