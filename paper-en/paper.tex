...
\begin{abstract}
I study the $(\infty, n)$-categorical moduli space $\fM$ of formal systems (hereafter \textbf{MSFS}): the classifying $2$-stack of Morita-equivalence classes of Rich-foundations ($\LFnd$ theories satisfying (R1)--(R5)). I establish four structural results and one boundary corollary.
...
\subsection{Worked example: the MSFS coordinate of standard real analysis}
\label{subsec:worked-example}

To illustrate the framework concretely, consider the corpus $P_{\mathrm{RA}}$ of standard undergraduate real analysis (Dedekind cuts, $\varepsilon$-$\delta$ convergence, Lebesgue measure, sequences and series, classical analysis on $\mathbb{R}^n$). Its MSFS coordinate $(\mathrm{Fw}, \nu, \tau)$ is computable as follows.
\begin{itemize}[nosep]
...
\end{itemize}
The triple $(\mathrm{Fw}, \nu, \tau)(P_{\mathrm{RA}}) = (\{[\ZFC], [\HoTT], [\Eff], [\mathrm{Bishop}], \ldots\},\; \omega,\; 1)$ records that real analysis lives in the multi-foundation, $\omega$-deep, intensionally refinable region of $\fM$ (at the level of $\sigma$-algebras; the full Borel / projective hierarchy would raise $\nu$ to $\omega_1$ and is excluded here). Automated cross-foundation transfer of $P_{\mathrm{RA}}$ between any two members of $\mathrm{Fw}(P_{\mathrm{RA}})$ is a structural Morita gauge, not a re-formalisation. The same procedure assigns coordinates to other corpora (number theory: $(\{[\ZFC], [\mathsf{PA}], \ldots\}, \omega, 1)$; algebraic topology: $(\{[\ZFC], [\HoTT]\}, \omega \cdot 2, 1)$; non-commutative geometry: $(\{[\NCG]\}, \omega \cdot 2, ?)$).
